...
output "step_1_cluster_details" {
  ...
  value = {
    ...
    cluster_security_group_id = module.nithin_shop_eks.cluster_security_group_id
    cluster_oidc_issuer_url   = module.nithin_shop_eks.cluster_oidc_issuer_url
...
output "step_1_configure_kubectl" {
  ...
  value       = "aws eks update-kubeconfig --region ${var.aws_region} --name ${module.nithin_shop_eks.cluster_name}"
}
...
output "step_4_nithin_shop_ui_details" {
  ...
  value = {
    ...
    check_pods_command      = "kubectl get pods -n nithin-shop"
    check_ingress_command   = "kubectl get ingress -A"
    services = {
      ui       = "nithin-shop-ui"
      cart     = "nithin-shop-cart"
      catalog  = "nithin-shop-catalog"
      checkout = "nithin-shop-checkout"
      orders   = "nithin-shop-orders"
    }
  }
}
...
output "complete_deployment_summary" {
  ...
  value       = <<-EOT
    ...
    STEP 1: CLUSTER DETAILS
    ------------------------
    Cluster Name: ${module.nithin_shop_eks.cluster_name}
    Region: ${var.aws_region}
    Version: ${module.nithin_shop_eks.cluster_version}
    Endpoint: ${module.nithin_shop_eks.cluster_endpoint}
    
    Configure kubectl:
    aws eks update-kubeconfig --region ${var.aws_region} --name ${module.nithin_shop_eks.cluster_name}
